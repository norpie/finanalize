...
<div class="flex flex-1 flex-col gap-4 p-4">
    <Dialog.Root bind:open={dialogOpen}>
        <Dialog.Trigger class=" text-white hover:text-[#9333ea] transition-all">New Report</Dialog.Trigger>
        ...
        {#if reports}
                ...
                <Table.Body>
                    {#each reports as report, i (i)}
                        <Table.Row onclick={() => goto(`/reports/${report.id}`)}>
                            ...
                            >
                                <Badge class={statusColor(report.status)}>{report.status}</Badge>
                            </Table.Cell
                            >
                            <Table.Cell>{formatDate(new Date(report.created_at))}</Table.Cell>
                            <Table.Cell
                                    class=" text-white hover:text-[#9333ea] transition-all">{report.user_input}</Table.Cell>
            ...
            >
                {#snippet children({pages, currentPage})}
                    <Pagination.Content>
                        <Pagination.Item>
                            <Pagination.PrevButton class="text-white hover:text-[#9333ea] transition-all">
                        ...
                        {/each}
                        <Pagination.Item>
                            <Pagination.NextButton class="text-white hover:text-[#9333ea] transition-all">
