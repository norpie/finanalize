<script lang="ts">
    ...
    function startStatusTimer(newStatus: string) {
        ...
        const savedStatusStartTime = localStorage.getItem(`reportStatusStartTime_${data.id}_${currentStatus}`);

        if (savedStatusStartTime) {
            statusTimerStart = parseInt(savedStatusStartTime, 10);
        } else {
            statusTimerStart = Date.now();
            localStorage.setItem(`reportStatusStartTime_${data.id}_${currentStatus}`, statusTimerStart.toString());
        }
...
<div class="m-4 p-4 space-y-4 flex justify-center">
    <Button onclick={retry} class="mb-6">Retry</Button>

    {#if report}
        <div class="flex flex-row justify-center p-4 w-full">
            <Card.Root class="w-full max-w-4xl md:max-w-5xl lg:max-w-6xl xl:max-w-7xl rounded-lg bg-gray-900 p-8 text-white shadow-lg space-y-6 flex flex-col items-center text-center">
                <!-- Report Title -->
                <Card.Title class="text-2xl font-bold text-white mb-4">
                ...
                <div class="space-y-2">
                    ...
                    <p class="text-sm text-gray-300">
                        <span class="font-semibold">Generation time:</span> {elapsedTime}
                    </p>
                </div>

                <Card.Content class="w-full flex flex-col items-center space-y-6">
                    ...
                    {#if verdict === 'Invalid'}
                        <div class="p-4 bg-red-800 rounded-md mb-4 w-full">
                    ...
                    {#if report.status !== 'Done' && report.status !== 'Invalid'}
                        <div class="space-y-4 mt-4 w-full flex flex-col items-center">
                            <!-- Progress Bar -->
                            <div class="relative w-full h-2.5 bg-gray-700 rounded-full">
                                <div
                            ...
                            <!-- Status Timer -->
                            <p class="text-sm text-gray-300">
                                <span class="font-semibold">{getStatusLabel(currentStatus)} started:</span> {statusElapsedTime}
                            </p>
                        </div>
                    {/if}
                </Card.Content>
