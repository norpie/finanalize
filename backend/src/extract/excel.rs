...
#[async_trait]
impl Extract for ExcelExtractor {
    async fn extract(&self, file_path: &str) -> crate::prelude::Result<()> {
        // Convert Excel to CSV
        let mut workbook: Xlsx<_> = open_workbook(file_path)?;
        let sheet_name = workbook.sheet_names().get(0).ok_or("No sheet found")?;
        let range = workbook.worksheet_range(sheet_name).ok_or("Cannot find sheet")??;
        ...
        csv_extractor.extract(temp_csv.path().to_str().unwrap()).await
    }
}
...
mod tests {
    ...
    async fn test_extract() {
        ...
        let mut workbook = calamine::Xlsx::<std::fs::File>::new(File::create(&file_path).unwrap()).unwrap();
