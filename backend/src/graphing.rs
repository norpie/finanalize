...
use std::env;
use plotters::style::full_palette::{RED_400, DEEPORANGE_500, BLUE_400, GREEN_400, YELLOW_400, CYAN_400, LIGHTBLUE_400};
...
fn create_histogram(histogram_data: HistogramData) -> Result<()> {
    ...
    let root = BitMapBackend::new(out_file_name, (900, 600)).into_drawing_area();
    root.fill(&WHITE)?;
    let x_values: Vec<u32> = histogram_data
        .x_values
        .iter()
        .map(|x| x.clone())
        .collect();
    ...
    let y_max = histogram_data
        ...
        .unwrap();
    let mut chart = ChartBuilder::on(&root)
        .caption(histogram_data.caption.as_str(), ("sans-serif", 50).into_font())
        .margin(5)
        .x_label_area_size(40)
        .y_label_area_size(40)
...
fn create_pie_chart(pie_chart_data: PieChartData) -> Result<()> {
    ...
    let colors = vec![RED_400, BLUE_400, GREEN_400, YELLOW_400, CYAN_400, MAGENTA, LIGHTBLUE_400];
    ...
    pie.percentages(("sans-serif", radius * 0.08).into_font().color(&BLACK.mix(0.5)));
    root.draw(&pie)?;
    root.present().expect("Unable to save result");
...
fn create_stock_chart(stock_data: StockChartData) -> Result<()> {
    ...
    let x_max = parse_time(&stock_data.dates[stock_data.dates.len() - 1]) + chrono::Duration::days(1);
    let mut chart = ChartBuilder::on(&root)
    ...
    chart.draw_series(
        stock_data.dates.iter().enumerate().map(|(i, date)| {
            CandleStick::new(
                parse_time(date),
                stock_data.open[i],
                stock_data.high[i],
...
mod tests {
    ...
    fn test_create_stock_chart() {
       let stock_data = StockChartData {
            dates: vec![
                "2024-02-10".to_string(), "2024-02-11".to_string(), "2024-02-12".to_string(),
                "2024-02-13".to_string(), "2024-02-14".to_string(), "2024-02-15".to_string(),
                "2024-02-16".to_string(), "2024-02-17".to_string(), "2024-02-18".to_string(),
                "2024-02-19".to_string(), "2024-02-20".to_string(),
            ],
            open: vec![120.0, 125.0, 122.0, 118.0, 121.0, 127.0, 124.0, 126.0, 122.5, 120.0, 121.5],
            high: vec![125.0, 128.0, 124.5, 119.0, 123.0, 130.0, 126.5, 127.0, 124.0, 122.0, 124.0],
            low: vec![118.0, 123.0, 120.0, 116.5, 119.5, 125.5, 122.0, 123.5, 120.5, 118.5, 119.0],
            close: vec![123.0, 126.5, 121.0, 117.5, 122.0, 128.0, 125.0, 124.0, 121.0, 119.5, 122.0],
